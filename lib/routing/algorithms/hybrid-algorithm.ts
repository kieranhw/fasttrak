...
import { calculateActualTravel, initialiseMetrics as computeMetrics } from '../../google-maps/directions';
...
/**
 * 10 step process to optimise the VRPSolution using a hybrid algorithm. A VRPSolution is generated by taking 
...
export async function hybridAlgorithm(graph: Graph, vehicles: Vehicle[], profile: ScheduleProfile): Promise<{ finalSolution: VRPSolution, scheduleReport: ScheduleReport }> {
    ...
    const kMeansOptimised = gaKMeansInit.evolve(NUM_GENERATIONS);
    ...
    const randomOptimised = gaRandomInit.evolve(NUM_GENERATIONS);
    ...
    for (const solution of [randomOnly, kMeansOnly[0], kMeansOptimised, randomOptimised]) {
        ...
        for (const route of solution.routes) {
            await calculateActualTravel(route);
        }
    }
